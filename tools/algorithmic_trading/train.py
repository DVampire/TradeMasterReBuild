...
def parse_args():
    ...
    parser.add_argument("--config",
                        default=osp.join(
                            ROOT, "configs", "algorithmic_trading",
...
def test_dqn():
    ...
    if task_name.startswith("style_test"):
        ...
        for i, path in enumerate(dataset.test_style_paths):
            test_style_environments.append(
    ...
    act_optimizer = build_optimizer(cfg,
                                    default_args=dict(params=act.parameters()))
    if cfg.cri:
        cfg.cri.update(dict(action_dim=action_dim, state_dim=state_dim))
        cri = build_net(cfg.cri)
        cri_optimizer = build_optimizer(
            cfg, default_args=dict(params=cri.parameters()))
    else:
        cri = None
        cri_optimizer = None
    ...
    agent = build_agent(cfg,
                        default_args=dict(action_dim=action_dim,
                                          state_dim=state_dim,
                                          act=act,
                                          cri=cri,
    ...
    if task_name.startswith("train"):
        ...
        print('win rate is: ',
              sum(r > 0 for r in daily_return_list) / len(daily_return_list))
