...
class AlgorithmicTradingDQN(AgentBase):
    def __init__(self, **kwargs):
        ...
        self.max_step = get_attr(
            kwargs, "max_step", 12345
        )  # the max step number of an episode. 'set as 12345 in default.
        self.state_dim = get_attr(
            kwargs, "state_dim",
            10)  # vector dimension (feature number) of state
        self.action_dim = get_attr(
            kwargs, "action_dim",
            2)  # vector dimension (feature number) of action
        ...
        self.cri_optimizer = get_attr(
            kwargs, "cri_optimizer", None) if get_attr(
                kwargs, "cri_optimizer", None) else self.act_optimizer
        self.act_optimizer.parameters = MethodType(get_optim_param,
                                                   self.act_optimizer)
        self.cri_optimizer.parameters = MethodType(get_optim_param,
                                                   self.cri_optimizer)
        """attribute"""
        if self.num_envs == 1:
        ...
        self.if_use_per = get_attr(
            kwargs, 'if_use_per',
            False)  # use PER (Prioritized Experience Replay)
        if self.if_use_per:
    ...
    def explore_one_env(self,
                        env,
                        horizon_len: int,
                        if_random: bool = False) -> Tuple[Tensor, ...]:
        """
        ...
        """
        states = torch.zeros((horizon_len, self.num_envs, self.state_dim),
                             dtype=torch.float32).to(self.device)
        actions = torch.zeros((horizon_len, self.num_envs, 1),
                              dtype=torch.int32).to(self.device)  # different
        rewards = torch.zeros((horizon_len, self.num_envs),
                              dtype=torch.float32).to(self.device)
        dones = torch.zeros((horizon_len, self.num_envs),
    ...
    def get_obj_critic_raw(self, buffer: ReplayBuffer,
        ...
        with torch.no_grad():
            ...
            next_q = self.cri_target(next_s).max(dim=1,
    ...
    def get_obj_critic_per(self, buffer: ReplayBuffer,
                           batch_size: int) -> Tuple[Tensor, Tensor]:
        """
        ...
        """
        with torch.no_grad():
            state, action, reward, undone, next_s, is_weights = buffer.sample(
                batch_size)
        ...
        td_error = self.criterion(
            q_value, q_label)  # or td_error = (q_value - q_label).abs()
    ...
    def optimizer_update_amp(self, optimizer: torch.optim,
                             objective: Tensor):  # automatic mixed precision
        """minimize the optimization objective via update the network parameters
    ...
    def update_avg_std_for_normalization(self, states: Tensor,
                                         returns: Tensor):
        ...
        self.cri.value_avg[:] = self.cri.value_avg * (1 -
                                                      tau) + returns_avg * tau
        self.cri.value_std[:] = self.cri.value_std * (
